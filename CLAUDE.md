...
- **Dev URLs / CORS:** dev is **plain HTTP end to end** — Kestrel listens only on `http://localhost:5144` (the sole `http` launch profile) and the SPA on `http://localhost:9000`. `Program.cs` must call `app.UseCors(SpaCorsPolicy)` and skip `UseHttpsRedirection` in Development, or the SPA login preflight fails. Allowed origins come from `Cors:AllowedOrigins` (`http://localhost:9000`).
- **Browser auto-open:** both apps open a browser on launch — `launchSettings.json` `launchBrowser: true` + `launchUrl: "scalar"` (API → `http://localhost:5144/scalar`, VS / `dotnet run --launch-profile http` only) and `quasar.config.js` `devServer.open: true` (SPA → `http://localhost:9000`).
- **No dev TLS:** HTTPS was deliberately removed from local dev. Consequence: the **Authorize.Net Accept.js and Square Web Payments SDKs refuse to run on an `http://` page**, so those two card-tokenization flows cannot be exercised locally. To restore, set `devServer.https`, point `WEB/config/env.dev.cjs` `API_BASE_URL` back at an https endpoint, add an https URL to the launch profile, and re-add `Kestrel:Certificates:Default`.
